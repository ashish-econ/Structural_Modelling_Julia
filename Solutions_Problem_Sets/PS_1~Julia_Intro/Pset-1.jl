...
function q3()
    ...
    ###What percent are college graduates
    mean(df."collgrad")  # ~ 23.68 %

    #= (c): Use thetabulatecommand to report what 
    percentage of the sample is in each racecategory=#
    by(df,:race,nrow)

    #=(d): Use describe function to create a matrix...are missing =#
    summarystats = describe(df,:all)
    ...
    a = df[:,[:occupation,:industry,:wage]] 
    by(a,[:industry,:occupation], a-> mean(a.wage))

    #(g): Wrap a fn around all of the code for question3
end
...
function q4()
    ...
    function matrixops(A,B)
        #= This function computes the element-wise product,addition,
            and normal dot product of two matrices =#
        if size(A) != size(B)
    ...
    # Part(d): Evaluate the above function using A,B from ques-1
    A,B,C,D = q1()
    matrixops(A,B)

    # Part(e): Add if-else condition in the function

    # Part(f): Evaluate the function with C and D matrix
    matrixops(C,D)
